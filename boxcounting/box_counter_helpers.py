...
class BoxCounterHelpers:
    ...
    def select_image(parent):
        ...
        if img is None:
            QMessageBox.warning(parent, "Box Counter", "Failed to load image.")
            return None
        return img, fname
    ...
    def apply_roi_size(parent, roi_text):
        try:
            ...
            QMessageBox.warning(parent, "Box Counter", "ROI size must be a positive integer.")
            return None
        return roi_size
    ...
    def update_display(parent):
        ...
        if parent.bc_point and parent.bc_roi_size:
            ...
            if parent.bc_last_fd is not None:
                cv2.putText(disp, f"D={parent.bc_last_fd}",
                            (x + parent.bc_roi_size + 5, y + 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                cv2.putText(disp, f"{parent.bc_roi_size}x{parent.bc_roi_size} t={parent.bc_last_time}s",
