...
# Model paths for different views (ONNX) - always resolve relative to this file
MODEL_DIR = os.path.join(os.path.dirname(__file__), "output_models")
MODEL_PATHS = [
    os.path.join(MODEL_DIR, "tumor_detector_axial.onnx"),
    os.path.join(MODEL_DIR, "tumor_detector_coronal.onnx"),
    os.path.join(MODEL_DIR, "tumor_detector_sagittal.onnx"),
]
...
def handle_tumor_image_double_click(self, idx: int) -> None:
    ...
    # Use package images/boxcounter as default directory for demo/sample images
    import os
    default_dir = _get_default_tumor_images_dir()
    # Fallback to images/boxcounter if originals does not exist
    if not os.path.isdir(default_dir) or not os.listdir(default_dir):
        try:
