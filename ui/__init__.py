# --- Utility for selecting images from package data ---
import importlib.resources
def get_images_folder_path():
    # Returns the absolute path to the images directory in the installed package
    return importlib.resources.files("images").__fspath__()
...
class MainWindow(QMainWindow):
    ...
    def setup_fractal_tab(self) -> None:
        ...
        def on_fractal_type_changed(s):
            combo_frac_type_changed(self, s)
            # generate_fractal_threaded(self)
        self.fractal_menu.currentTextChanged.connect(on_fractal_type_changed)
        ...
        # Number of Iterations input (row 7)
        self.iter_label = QLabel("<b>Number of Iterations:</b>")
        self.iter_label.setStyleSheet("color: #0a2342; font-size: 15px;")
        self.iter_label.setToolTip("Set the number of iterations (1-100) for fractal generation. Overrides Max Iterations if set.")
        self.iter_entry = QLineEdit()
        self.iter_entry.setPlaceholderText("e.g. 50")
        self.iter_entry.setToolTip("Enter a value between 1 and 100. Overrides Max Iterations if set.")
        self.iter_entry.setAccessibleName("Number of Iterations Input")
        self.iter_entry.setStyleSheet("QLineEdit { background: #fff; border: 1.5px solid #1976d2; border-radius: 7px; padding: 7px 11px; font-size: 15px; color: #0a2342; }")
        controls_layout.addWidget(self.iter_label, 7, 0)
        controls_layout.addWidget(self.iter_entry, 7, 1)
        ...
        def generate_fractal_with_iterations():
            iter_text = self.iter_entry.text().strip()
            if iter_text:
                try:
                    iter_val = int(iter_text)
                    if not (1 <= iter_val <= 100):
                        QMessageBox.warning(self, "Invalid Iterations", "Number of Iterations must be between 1 and 100.")
                        return
                except ValueError:
                    QMessageBox.warning(self, "Invalid Iterations", "Please enter a valid integer for Number of Iterations.")
                    return
                self.max_iter = iter_val
            else:
                try:
                    max_iter_val = int(self.max_iter_entry.text().strip())
                    self.max_iter = max_iter_val
                except Exception:
                    pass
            generate_fractal_threaded(self)
        self.generate_button.clicked.connect(generate_fractal_with_iterations)
        ...
        # --- Improved Zoom/Reset Toolbar (Vertical, User-Friendly) ---
        zoom_toolbar = QVBoxLayout()
        zoom_toolbar.setSpacing(10)
        zoom_toolbar.setContentsMargins(0, 0, 0, 0)

        zoom_toolbar.addStretch(1)  # Top stretch for centering

        self.zoom_in_btn = QPushButton("+")
        self.zoom_in_btn.setToolTip("Zoom In (Smooth)")
        self.zoom_in_btn.setFixedSize(40, 40)
        self.zoom_in_btn.setStyleSheet("font-size: 22px; font-weight: bold; background: #e3f2fd; border-radius: 20px; border: 2px solid #1976d2; color: #1976d2;")
        self.zoom_in_btn.clicked.connect(self.fractal_zoom_in)
        zoom_toolbar.addWidget(self.zoom_in_btn)

        self.zoom_out_btn = QPushButton("–")
        self.zoom_out_btn.setToolTip("Zoom Out (Smooth)")
        self.zoom_out_btn.setFixedSize(40, 40)
        self.zoom_out_btn.setStyleSheet("font-size: 22px; font-weight: bold; background: #e3f2fd; border-radius: 20px; border: 2px solid #1976d2; color: #1976d2;")
        self.zoom_out_btn.clicked.connect(self.fractal_zoom_out)
        zoom_toolbar.addWidget(self.zoom_out_btn)

        self.reset_zoom_btn = QPushButton("⟳")
        self.reset_zoom_btn.setToolTip("Reset Zoom to Original View")
        self.reset_zoom_btn.setFixedSize(40, 40)
        self.reset_zoom_btn.setStyleSheet("font-size: 20px; font-weight: bold; background: #e3f2fd; border-radius: 20px; border: 2px solid #43aa8b; color: #388e3c;")
        self.reset_zoom_btn.clicked.connect(self.fractal_reset_zoom)
        zoom_toolbar.addWidget(self.reset_zoom_btn)

        zoom_toolbar.addStretch(1)  # Bottom stretch for centering
        ...
        # Add zoom toolbar vertically to the right of the canvas
        canvas_toolbar_row = QHBoxLayout()
        canvas_toolbar_row.setContentsMargins(0, 0, 0, 0)
        canvas_toolbar_row.setSpacing(0)
        canvas_toolbar_row.addWidget(self.canvas, stretch=100)
        canvas_toolbar_row.addLayout(zoom_toolbar)
        right_vbox.addLayout(canvas_toolbar_row)
    ...
    def fractal_zoom_in(self):
        # Zoom in: scale axis range by 0.9 (10% closer)
        if self.ax:
            ...
            xhalf = (xlim[1] - xlim[0]) * zoom_factor / 2
            yhalf = (ylim[1] - ylim[0]) * zoom_factor / 2
            self.ax.set_xlim(xmid - xhalf, xmid + xhalf)
            self.ax.set_ylim(ymid - yhalf, ymid + yhalf)
            self.canvas.draw_idle()

    def fractal_zoom_out(self):
        # Zoom out: scale axis range by 1.1 (10% farther)
        if self.ax:
            xlim = self.ax.get_xlim()
            ylim = self.ax.get_ylim()
            zoom_factor = 1.1
    ...
    def fractal_reset_zoom(self):
        # Reset to original axis limits
        if self.ax and self._original_xlim and self._original_ylim:
            self.ax.set_xlim(self._original_xlim)
            self.ax.set_ylim(self._original_ylim)
            self.canvas.draw_idle()

    # Patch: Save original axis limits after fractal is generated
    def after_fractal_draw(self):
        if self.ax:
    ...
    def setup_box_counter_tab(self):
        ...
        # Status label for Box Counter
        self.bc_status = QLabel("")
        self.bc_status.setStyleSheet("font-size: 12pt; color: #ffb300; font-weight: bold; background: #fffbe7; border-radius: 7px; padding: 6px 12px 6px 12px; margin-bottom: 4px; border: 1.5px solid #ffe066;")
        self.bc_status.setAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
        ...
        # Add with maximum vertical stretch
        layout.addWidget(self.bc_image_label, stretch=100)
    ...
    def handle_image_label_double_click(self, which):
        ...
        try:
            ...
            import os
            start_dir = get_images_folder_path()
            if which == 1:
                file_path, _ = QFileDialog.getOpenFileName(self, "Select Image 1", start_dir, "Images (*.png *.jpg *.jpeg *.bmp *.tif *.tiff *.gif)")
                if file_path and not os.path.exists(file_path):
                    # Fallback: try resolving relative to images package
                    fallback_path = os.path.join(get_images_folder_path(), os.path.basename(file_path))
...
def main():
    ...
    # Visually appealing tooltip style
    app.setStyleSheet("""
        QToolTip, QLineEdit QToolTip, QComboBox QToolTip, QAbstractItemView QToolTip {
            color: #111 !important;
