...
type Config struct {
	...
	OutputDir string // where WAV chunk files are written
}
...
// Run reads 16-bit PCM (s16le), writes active speech to per-chunk WAV files (if configured),
	...
	for {
		...
		case !inSpeech && !isSilence:
			...
			if cfg.OutputDir != "" {
				...
				w, err := NewWavWriter(curPath, cfg.SampleRate, cfg.Channels, 16)
				if err != nil {
					return err
				}
				curWriter = w
...
func NewWavWriter(path string, sampleRate, channels, bitsPerSample int) (*wavWriter, error) {
